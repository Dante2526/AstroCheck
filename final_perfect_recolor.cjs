...
async function processImage(inputFile, outputFile, hasHeart) {
  ...
  for (let y = 0; y < img.height; y++) {
    for (let x = 0; x < img.width; x++) {
      ...
      // Heart
      if (isHeartArea && (h < 0.05 || h > 0.9) && s > 0.25) {
        newH = 1.0; 
        newS = 1.0; 
        newL = Math.max(0.3, Math.min(0.6, l)); // Keep shading, but make it very saturated red
      } 
      // Thermometer Screen (Cyan/Blue)
      else if (hasHeart && h > 0.45 && h < 0.70 && s > 0.1) {
        newS = s * 1.5; // just boost it a bit so it's clearly blue
      }
      // Orange Suit Parts (includes all warm hues that aren't the heart/moon)
      else if (s > 0.15 && (h >= 0.00 && h < 0.15)) {
        newH = 0.0694; // Target #FF6B00
        newS = 1.0;
        newL = 0.5 + (l - 0.5) * 0.8;
      } 
      // Yellow Moon
      else if (!hasHeart && h >= 0.12 && h < 0.20 && s > 0.3) {
        newH = 0.15;
        newS = 1.0;
      }
      // Visor or dark shadows
      else if (l < 0.35 || (l < 0.45 && s < 0.2)) {
        newS = 0;
        newL = Math.max(0.02, Math.min(0.2, l * 0.7)); // Visor Escuro #1A1A1A
      } 
      // White suit base
      else {
        newS = 0; // Branco Puro #FFFFFF
        newL = Math.min(1, l * 1.25); // Brighten to pure white
      }
