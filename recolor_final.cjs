...
async function processImage(inputFile, outputFile) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if ((h < 0.025 || h > 0.95) && s > 0.35) {
      h = 1.0; 
      s = Math.min(1, s * 1.5); 
      l = Math.max(0.3, Math.min(0.6, l));
    } 
    else if (h >= 0.025 && h < 0.12 && s > 0.25) {
      h = 0.07; 
      s = Math.min(1, s * 1.5);
    } 
...
(async () => {
  await processImage('/tmp/astro1_orig.jpg', 'public/astronaut1_colored.png');
  console.log('Saved to public/astronaut1_colored.png');
  await processImage('/tmp/astro2_orig.jpg', 'public/astronaut2_colored.png');
