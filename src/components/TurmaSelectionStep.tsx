...
export const TurmaSelectionStep: React.FC<TurmaSelectionStepProps> = ({
  ...
  if (isSuccess && selectedTurma) {
    ...
    return (
      <div className="w-full bg-surface-container-lowest dark:bg-[#1E2029] rounded-2xl overflow-y-auto custom-scrollbar flex flex-col items-center text-center transition-all duration-300 shadow-[0_4px_20px_rgba(32,59,139,0.10)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.5)] border-[3px] border-[#22c55e] dark:border-[#22c55e] p-3.5 sm:p-7 flex-1 min-h-0 max-h-[540px] sm:max-h-[580px] justify-between">
        ...
        <div className="my-1 sm:my-2">
          ...
          <h2 className="text-base sm:text-2xl font-black text-on-surface dark:text-[#f7fafc] leading-tight">
            Prontidão Enviada com Sucesso!
          </h2>

          <p className="text-[11px] sm:text-sm text-on-surface-variant dark:text-[#a0aec0] mt-1 sm:mt-2 max-w-md mx-auto leading-relaxed">
          ...
          <div className="mt-2 sm:mt-3 py-1.5 sm:py-2 px-3 sm:px-4 rounded-xl bg-surface-container-low/60 dark:bg-[#171922] border border-outline-variant/60 dark:border-[#2d3139] flex items-center justify-center gap-2 sm:gap-3">
            <span className="text-[11px] sm:text-xs font-semibold text-on-surface dark:text-[#f7fafc]">
              Status de Aptidão:
            </span>
  ...
  return (
    <div className="w-full bg-surface-container-lowest dark:bg-[#1E2029] rounded-2xl flex flex-col relative transition-all duration-300 shadow-[0_4px_20px_rgba(32,59,139,0.10)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.5)] border-[3px] border-transparent dark:border-[#2d3139] p-3 sm:p-5 flex-1 min-h-0 max-h-[520px] sm:max-h-[580px] overflow-hidden justify-between">
      ...
      <div className="flex items-center gap-2.5 sm:gap-3 pb-1.5 sm:pb-2 border-b border-outline-variant/40 dark:border-[#2d3139] shrink-0">
        <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-xl bg-surface-container-low/60 dark:bg-[#171922] p-1 flex items-center justify-center shrink-0 border border-outline-variant/40 dark:border-[#2d3139]">
          <img 
        ...
        <div className="text-left flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-[#ff6b00] dark:text-[#ff7a00]">
              Etapa Final
            </span>
            <span className="text-xs text-on-surface-variant dark:text-[#718096]">&bull;</span>
            <span className="text-[10px] sm:text-xs text-on-surface-variant dark:text-[#a0aec0]">
              Destino do E-mail
            </span>
          </div>
          <h2 className="text-xs sm:text-base font-bold text-on-surface dark:text-[#f7fafc] truncate">
      ...
      {/* Grid de Seleção das 4 Turmas (com rolagem suave se altura for muito restrita) */}
      <div className="grid grid-cols-2 gap-2 sm:gap-3.5 my-1.5 sm:my-2.5 flex-1 items-center overflow-y-auto custom-scrollbar py-0.5">
        {ALL_TURMA_KEYS.map((key) => {
          ...
          return (
            ...
            >
              ...
              <div className={`absolute top-2 right-2 sm:top-2.5 sm:right-2.5 w-4 h-4 sm:w-6 sm:h-6 rounded-full flex items-center justify-center transition-all duration-200 ${
                isSelected 
      ...
      <div className="min-h-[32px] sm:min-h-[38px] px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-surface-container-low/50 dark:bg-[#171922]/80 border border-outline-variant/40 dark:border-[#2d3139] flex items-center justify-between text-[11px] sm:text-xs text-on-surface-variant dark:text-[#a0aec0] shrink-0">
        ...
        {selectedTurma && (
          <span className="text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded bg-surface-container-high dark:bg-[#252836] text-primary dark:text-[#90eff9] shrink-0">
      ...
      <div className="w-full flex justify-between items-center mt-2 sm:mt-3 pt-1.5 sm:pt-2 border-t border-outline-variant/40 dark:border-[#2d3139] shrink-0">
        ...
        <button 
          ...
          className={`text-xs sm:text-sm font-bold py-2 sm:py-3 px-4 sm:px-7 rounded-full shadow-sm transition-all duration-200 flex items-center gap-2 ${
            isSendDisabled
        ...
        >
          {isSending ? (
            <>
              <span className="w-3.5 h-3.5 sm:w-4 sm:h-4 border-2 border-white/40 border-t-white rounded-full animate-spin"></span>
